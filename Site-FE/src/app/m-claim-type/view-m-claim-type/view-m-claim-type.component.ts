...
@Component({
  selector: 'app-view-m-claim-type',
  templateUrl: './view-m-claim-type.component.html',
  styleUrls: ['./view-m-claim-type.component.scss']
})
export class ViewMClaimTypeComponent implements OnInit {
  ...
  columnDefs = [
    { field: 'type', headerName: 'Tipo de Reclamacion', colId: "0", width: 400, editable: true, filter: "agTextColumnFilter", filterParams: DataGridFunctions.CodeFilterParams},
    {
      field: "Gestionar",
      headerName: "",
      width: 200,
  ...
  ngOnInit(): void {
    ...
    this.claimTypeService.getClaimType().subscribe((res: any) => {
      console.log(res);
      for(let i of res) {

        this.claimType.ID = parseInt(i[0]);
        this.claimType.type = i[1];
        this.claimTypes.push(this.claimType)
        this.claimType = new Claim_Type();
      }
      this.rowData = this.claimTypes;
    });
  }
  ...
  onDeleteRow() {
    ...
    for(let row of selectedRows) {
      this.claimTypeService.deleteClaimType(selectedRows[0].ID).subscribe(res => {
        this.gridOptions.api.applyTransaction({ remove: selectedRows });
      }, err => {
        window.alert("Este tipo de queja esta en uso")
      });
    }
  }
  ...
  onSaveChanges() {
    ...
    for (let x of modifiedRows) {
      ...
      this.claimType.type = this.gridOptions.api.getValue("0", row2);
      this.claimType.ID = this.gridOptions.api.getValue("3", row2);
      console.log(JSON.parse(JSON.stringify(this.claimType)))
      this.claimTypeService.updateClaimType(this.claimType).subscribe(res => {
    ...
    //     this.claimTypeService.getclaimTypeById(this.gridOptions.api.getValue("9", row4)).subscribe(res => {
    //       this.job.job_claimType = res;
