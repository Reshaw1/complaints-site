...
@Component({
  selector: 'app-view-m-complain-type',
  templateUrl: './view-m-complain-type.component.html',
  styleUrls: ['./view-m-complain-type.component.scss']
})
export class ViewMComplainTypeComponent implements OnInit {
  ...
  columnDefs = [
    { field: 'type', headerName: 'Tipo de Queja', colId: "0", width: 400, editable: true, filter: "agTextColumnFilter", filterParams: DataGridFunctions.CodeFilterParams},
    {
      field: "Gestionar",
      headerName: "",
      width: 200,
  ...
  ngOnInit(): void {
    ...
    this.complainTypeService.getComplainType().subscribe((res: any) => {
      console.log(res);
      for(let i of res) {

        this.complainType.ID = parseInt(i[0]);
        this.complainType.type = i[1];
        this.complainTypes.push(this.complainType)
        this.complainType = new Complain_Type();
      }
      this.rowData = this.complainTypes;
    });
  }
  ...
  onAddRow() {
    ...
    //     job_State: "Vacante",
    //     Modified: "New",
    //     job_complainType: { complainType_Name: "",
    //                       complainType_Id: ""
    //                     },
    //     job_Id: "",
    //   },
  ...
  onDeleteRow() {
    ...
    for(let row of selectedRows) {
      this.complainTypeService.deleteComplainType(selectedRows[0].ID).subscribe(res => {
        this.gridOptions.api.applyTransaction({ remove: selectedRows });
      }, err => {
        window.alert("Este tipo de reclamacion esta en uso")
      });
    }
  }
  ...
  onSaveChanges() {
    ...
    for (let x of modifiedRows) {
      ...
      this.complainType.type = this.gridOptions.api.getValue("0", row2);
      this.complainType.ID = this.gridOptions.api.getValue("3", row2);
      console.log(JSON.parse(JSON.stringify(this.complainType)))
      this.complainTypeService.updateComplainType(this.complainType).subscribe(res => {
    ...
    //     this.complainTypeService.getcomplainTypeById(this.gridOptions.api.getValue("9", row4)).subscribe(res => {
    //       this.job.job_complainType = res;
