...
@Component({
  selector: 'app-view-m-claim-state',
  templateUrl: './view-m-claim-state.component.html',
  styleUrls: ['./view-m-claim-state.component.scss']
})
export class ViewMClaimStateComponent implements OnInit {
  ...
  columnDefs = [
    { field: 'state', headerName: 'Estado de Reclamacion', colId: "0", width: 400, editable: true, filter: "agTextColumnFilter", filterParams: DataGridFunctions.CodeFilterParams},
    {
      field: "Gestionar",
      headerName: "",
      width: 200,
  ...
  ngOnInit(): void {
    ...
    this.claimStateService.getClaimState().subscribe((res: any) => {
      console.log(res);
      for(let i of res) {

        this.claimState.ID = parseInt(i[0]);
        this.claimState.state = i[1];
        this.claimStates.push(this.claimState)
        this.claimState = new Claim_State();
      }
      this.rowData = this.claimStates;
    });
  }
  ...
  onAddRow() {
    ...
    //     job_State: "Vacante",
    //     Modified: "New",
    //     job_claimState: { claimState_Name: "",
    //                       claimState_Id: ""
    //                     },
    //     job_Id: "",
    //   },
  ...
  onDeleteRow() {
    ...
    for(let row of selectedRows) {
      this.claimStateService.deleteClaimState(selectedRows[0].ID).subscribe(res => {
        this.gridOptions.api.applyTransaction({ remove: selectedRows });
      }, err => {
        window.alert("Este estado de reclamacion esta en uso")
      });
    }
  }
  ...
  onSaveChanges() {
    ...
    for (let x of modifiedRows) {
      ...
      this.claimState.state = this.gridOptions.api.getValue("0", row2);
      this.claimState.ID = this.gridOptions.api.getValue("3", row2);
      console.log(JSON.parse(JSON.stringify(this.claimState)))
      this.claimStateService.updateClaimState(this.claimState).subscribe(res => {
    ...
    //     this.claimStateService.getclaimStateById(this.gridOptions.api.getValue("9", row4)).subscribe(res => {
    //       this.job.job_claimState = res;
